...
const ColorSwatch = ({ words, isFlipped, isReversed }: ColorSwatchProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative rounded-2xl  flex flex-col gap-4 h-98 w-30 text-2xl items-start justify-center overflow-hidden md:pl-10 md:-mt-4 -z-10",
      ...
      {words?.map((word, i) => (
        <motion.div
          ...
          initial={{ x: -100 * (isReversed ? -1 : 1) }}
          animate={{ x: isFlipped ? 0 : -100 * (isReversed ? -1 : 1) }}
          transition={{
